...
void ICACHE_FLASH_ATTR wifiInit() {
	...
	len = os_sprintf(tempSSID,"ZIGATE-%02X%02X", mac_addr[4], mac_addr[5]);
  
	os_memset(apconf.ssid, 0, 32);
	os_memcpy(apconf.ssid, tempSSID, len);
	// Specify the length of ssid
	apconf.ssid_len= len;
	
	len = os_sprintf(tempPass,"admin%02X%02X", mac_addr[4], mac_addr[5]);
	os_memset(apconf.password, 0, 64);
	os_memcpy(apconf.password, tempPass, len);
